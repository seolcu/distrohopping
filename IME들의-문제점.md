...
- KDE에서 "가상 키보드" 이용하지 말 것. XWayland에서 한글입력을 할 때 계속 잘못된 입력이 들어감.
- KDE에서 Fcitx5 사용시 kcm이 설치되어 있지 않다면 설치하면 좋다
- NixOS에서는 환경변수를 따로 설정할 필요 없이, 위키 따라서 enable만 해주면 됨.
- Fedora는 im-chooser가 설치되어 있으므로, XMODIFIERS 환경변수를 따로 지정하는 대신 "입력 방식" 프로그램으로 ime를 선택한다
...
- 한글모드로 바꿀때마다 Korean이라고 툴바가 뜸(끄는법: 전역 옵션 -> 입력기 변경시 입력기 정보 표시 체크해제하기)
